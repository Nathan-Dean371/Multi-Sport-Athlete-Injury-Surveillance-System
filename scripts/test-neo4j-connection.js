...
if (config.password === "your-password-here") {
  console.error(
    `${colors.red}✗${colors.reset} NEO4J_PASSWORD is not set. Pass it via env or update the default.`,
  );
  process.exit(1);
}
  ...
  try {
    ...
    try {
      ...
      result2.records.forEach((rec) => {
        info(
          `${rec.get("name")}: ${rec.get("version")} (${rec.get("edition")})`,
        );
      });
      ...
      if (result5.records.length > 0) {
        ...
        warning(
          "No data found. Run .\\scripts\\seed-dev-data.ps1 to add dev test data!",
        );
      }
      ...
      const playerTeamCount = result6.records[0]
        .get("playerTeamCount")
        .toNumber();
      ...
      if (queryTime < 100) {
        ...
        warning(
          `Query executed in ${queryTime}ms (consider optimizing indexes)`,
        );
      }
      ...
      if (result5.records.length === 0) {
        info(
          `${constraintCount === 0 ? "2" : "1"}. Run .\\scripts\\seed-dev-data.ps1 to add dev test data`,
        );
      }
      info(
        `${constraintCount === 0 ? (result5.records.length === 0 ? "3" : "2") : result5.records.length === 0 ? "2" : "1"}. Configure your backend to use these connection details`,
      );
      info(
        `${constraintCount === 0 ? (result5.records.length === 0 ? "4" : "3") : result5.records.length === 0 ? "3" : "2"}. Start building your NestJS services!`,
      );
    } finally {
      await session.close();
    }
    ...
    if (err.code === "ServiceUnavailable") {
      ...
      info(
        "  3. For new installations, default is neo4j/neo4j (you'll be prompted to change)",
      );
