...
#### 3. Injuries Domain (3 endpoints)
**Implementation Order: 3rd**
- `POST /injuries/` - Report new injury **CRITICAL PATH**
- `GET /injuries/:injuryId` - Get injury details
- `PATCH /injuries/:injuryId` - Update injury status and details **CRITICAL PATH**

**Rationale**: Core feature of the application. Players report injuries, coaches need to see them, and injuries need status tracking.

**Implementation Details**:
- Auto-generated injury IDs in format `INJ-YYYY-###` (year-based incrementing)
- Comprehensive injury tracking (bodyPart, side, severity, mechanism, diagnosis, treatmentPlan)
- Severity levels: Minor, Moderate, Severe, Critical
- Status tracking: Active, Recovering, Recovered, Chronic, Re-injured
- Status updates stored as separate nodes with history
- Creates Injury node and HAS_INJURY relationship
- Guard: JwtAuthGuard (players and medical staff can report)

**Neo4j Queries**:
```cypher
// Create injury with auto-generated ID
MATCH (p:Player {playerId: $playerId})
CREATE (i:Injury {
  injuryId: $injuryId,  // Format: INJ-YYYY-###
...
// Update injury and create status update
...
```json
{
  ...
  "severity": "Minor",
  "status": "Recovering",
  "injuryDate": "2024-01-14T10:00:00Z",
  "expectedReturnDate": "2024-01-21",
  "mechanism": "Landing from jump",
  "diagnosis": "Grade 1 lateral ankle sprain",
  "treatmentPlan": "RICE protocol, light mobility exercises",
  "notes": "Occurred during practice",
  "createdAt": "2026-01-14T22:48:29.152Z",
  "updatedAt": "2026-01-14T22:50:44.697Z",
  "player": {
    "playerId": "PLAYER-003",
    "diagnosedDate": "2026-01-14T22:48:29.681Z",
    "reportedBy": "PSY-PLAYER-A1B2C3D4"
  },
  "statusUpdates": [{
    "updateId": "UPDATE-1768431044692",
    "status": "Recovering",
    "notes": "Patient responding well to treatment",
    "recordedBy": "PSY-PLAYER-A1B2C3D4",
    "recordedAt": "2026-01-14T22:50:44.697Z"
  }]
}
```
...
- Notification service for alerting coaches (future enhancement)
...
**Injuries**:
- ~~`PATCH /injuries/:injuryId`~~ - **MOVED TO MVP (Phase 1)**
- `POST /injuries/:injuryId/resolve` - Mark injury as resolved (optional, status can be updated via PATCH)
...
│   ├── injuries.service.ts
│   └── dto/
│       ├── injury.dto.ts  # CreateInjuryDto, UpdateInjuryDto, InjuryDetailDto
│       │                  # InjurySeverity enum, InjuryStatus enum
│       └── (actual implementation uses single DTO file)
├── coaches/
│   ├── coaches.module.ts
