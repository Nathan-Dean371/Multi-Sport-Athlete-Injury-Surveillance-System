...
export class InjuriesService {
    ...
    try {
      ...
      if (playerCheck.records.length === 0) {
        throw new NotFoundException(
          `Player ${createInjuryDto.playerId} not found`,
        );
      }
      ...
      throw new BadRequestException(
        `Failed to create injury: ${error.message}`,
      );
    } finally {
      await session.close();
    }
  ...
  async updateInjury(
    ...
    try {
      ...
      throw new BadRequestException(
        `Failed to update injury: ${error.message}`,
      );
    } finally {
      await session.close();
    }
  ...
  async findAll(
    ...
    try {
      ...
      if (userRole === "player") {
        ...
        // Coaches can see injuries for players on teams they manage, and also players
        // connected via parents they invited.
        matchClause =
          "MATCH (c:Coach {pseudonymId: $userPseudonym}) MATCH (p:Player)-[r:SUSTAINED]->(i:Injury)";
        whereClauses.push(
          "(EXISTS { MATCH (c)-[:MANAGES]->(:Team)<-[:PLAYS_FOR]-(p) } OR EXISTS { MATCH (c)-[:INVITED_PARENT]->(:Parent)-[:PARENT_OF]->(p) })",
        );
        params.userPseudonym = userPseudonym;
      ...
      const sortField =
        sortBy === "injuryDate" ? "i.injuryDate" : `i.${sortBy}`;
      ...
      console.log(
        `Fetched ${identityMap.size} player identities from PostgreSQL for ${pseudonymIds.length} injuries`,
      );
      ...
      throw new BadRequestException(
        `Failed to fetch injuries: ${error.message}`,
      );
    } finally {
      await session.close();
    }
  ...
  async resolveInjury(
    ...
    try {
      ...
      throw new BadRequestException(
        `Failed to resolve injury: ${error.message}`,
      );
    } finally {
      await session.close();
    }
